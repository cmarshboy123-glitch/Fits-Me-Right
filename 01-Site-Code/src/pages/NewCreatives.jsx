      ...
      <section className="border-b border-black bg-[#1C1711] text-white">
        <div className="mx-auto grid max-w-[1440px] lg:grid-cols-[1.05fr_.95fr]">
          <div className="flex min-h-[590px] flex-col justify-center px-5 py-20 lg:px-12 lg:py-28">
            ...
            <div className="mt-10 flex flex-wrap gap-3"><a href="#spotlights" className="flex min-h-12 items-center gap-3 bg-gold-400 px-6 text-xs font-black text-black">MEET THE DESIGNERS <ArrowRight size={16} /></a><a href="#selection" className="flex min-h-12 items-center border border-white/40 px-6 text-xs font-black">HOW WE CURATE</a></div>
          </div>
  ...
  return (
    <article className="border border-white/25 bg-black">
      <div className={`runway-snippet relative aspect-[16/10] overflow-hidden ${paused ? 'is-paused' : ''}`}>
        ...
        <button type="button" onClick={() => setPaused((value) => !value)} aria-label={`${paused ? 'Play' : 'Pause'} ${dispatch.city} runway snippet`} className="absolute right-4 top-12 grid h-11 w-11 place-items-center rounded-full border border-white/50 bg-black/40 backdrop-blur-sm transition hover:bg-gold-400 hover:text-black">
        ...
        <div className="absolute inset-x-0 bottom-0 h-1 bg-white/20"><span className="runway-progress block h-full bg-gold-400" /></div>
      </div>
