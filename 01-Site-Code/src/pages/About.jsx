...
export default function About() {
  return (
    <section className="mx-auto max-w-[1440px] px-4 py-14 lg:px-8 lg:py-20">
      <p className="text-xs font-bold uppercase tracking-widest text-gold-700">About Fits Me Right</p>
      ...
      <div className="mt-12 grid border border-black md:grid-cols-3">
        {promises.map(({ icon: Icon, title, copy }, index) => (
          <div key={title} className="border-b border-black p-7 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0 lg:p-9">
            <div className="flex items-center justify-between"><Icon size={22} /><span className="text-xs font-black text-neutral-400">0{index + 1}</span></div>
      ...
      <div className="mt-14 rounded-[2rem] bg-gold-50 p-8 lg:p-12">
      ...
      <div className="mt-14 rounded-[2rem] bg-black p-8 text-white lg:p-12">
        ...
        <Link to="/style/gender" className="mt-7 inline-flex items-center gap-3 rounded-full bg-gold-400 px-7 py-4 text-sm font-black text-black">START YOUR STYLE EDIT <ArrowRight size={17} /></Link>
      </div>
    </section>
  )
}
