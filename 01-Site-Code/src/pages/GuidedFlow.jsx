...
export default function GuidedFlow() {
    ...
    return (
      <section className="mx-auto min-h-[70vh] max-w-[1440px] px-4 py-10 lg:px-8">
        <div className="flex flex-col gap-4 border-b border-black pb-7 sm:flex-row sm:items-end sm:justify-between">
          <div><p className="text-xs font-bold uppercase tracking-widest text-gold-700">Your personal edit</p><h1 className="mt-2 text-4xl font-black tracking-tight">Made to fit your life.</h1><p className="mt-3 text-sm text-neutral-600">{summarizeSelections(selections, ['gender', 'bodyType', () => selectedPrice?.title, (s) => s.shirtSize && `Shirt ${s.shirtSize}`, (s) => s.pantsSize && `Pants ${s.pantsSize}`, 'dressCode'])}</p></div>
        ...
        {!loading && results.length > 0 && <><div className="mb-7 border-y border-black py-5"><p className="text-[10px] font-black uppercase tracking-[.24em] text-gold-800">Verified direct matches</p><div className="mt-3 flex flex-wrap gap-x-6 gap-y-2 text-sm font-bold">{[...new Set(results.map((product) => product.vendor))].map((vendor) => <span key={vendor}>{vendor}</span>)}</div><p className="mt-3 max-w-2xl text-xs leading-5 text-neutral-500">Each button opens that exact retailer product page. Prices and size availability {latestVerifiedLabel ? `were checked ${latestVerifiedLabel}` : 'are a snapshot'} and can change at the store.</p></div><ProductGrid products={results} /></>}
        {!loading && results.length === 0 && <div className="grid place-items-center rounded-3xl bg-neutral-100 px-6 py-24 text-center"><div><p className="text-xs font-bold uppercase tracking-widest text-gold-700">The edit is still in progress</p><h2 className="mt-3 text-3xl font-black">No exact matches””yet.</h2><p className="mx-auto mt-3 max-w-md text-neutral-600">Try adjusting one detail and we’ll widen the rail.</p><button onClick={() => navigate('/style/dress-code')} className="mt-7 rounded-full bg-black px-7 py-4 text-sm font-bold text-white">CHANGE DRESS CODE</button></div></div>}
      </section>
    )
  }
  ...
  return (
    <section className="mx-auto min-h-[70vh] max-w-6xl px-4 py-10 lg:px-8 lg:py-16">
      <div className="mb-12"><div className="mb-6 flex items-center justify-between text-xs font-bold"><span>YOUR STYLE EDIT</span><span>{index + 1} / {filterStepCount}</span></div><div className="h-1 overflow-hidden rounded-full bg-neutral-200"><div className="h-full bg-gold-400 transition-all" style={{ width: `${((index + 1) / filterStepCount) * 100}%` }} /></div></div>
      ...
      {step === 'body-type' && <div className="grid border-y border-black lg:grid-cols-[260px_1fr]">
        <aside className="border-b border-black bg-[#F4EEE2] p-6 lg:border-b-0 lg:border-r lg:p-8">
          <p className="text-[11px] font-bold uppercase tracking-[.22em] text-gold-800">A note on fit</p>
      ...
      {step === 'save-profile' && <div className="grid border-y border-black lg:grid-cols-[1fr_1.15fr]">
        <div className="border-b border-black bg-[#F4EEE2] p-7 lg:border-b-0 lg:border-r lg:p-10">
          <p className="text-[10px] font-black uppercase tracking-[.24em] text-gold-800">Current preferences</p>
        ...
        <div className="p-7 lg:p-10">
          ...
          {profileSaved && <p className="mt-4 flex items-center gap-2 text-sm font-bold text-gold-800"><Check size={17} /> Profile saved. You can create another or continue to results.</p>}
          ...
          <button onClick={() => navigate('/style/results')} className="mt-8 flex w-full items-center justify-center gap-3 bg-gold-400 px-6 py-4 text-xs font-black">VIEW MY RESULTS <ArrowRight size={16} /></button>
        </div>
      </div>}
      </div>
    </section>
